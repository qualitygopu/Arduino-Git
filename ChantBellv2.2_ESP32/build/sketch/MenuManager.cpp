#line 1 "e:\\Arduino\\Arduino Git\\ChantBellv2.2_ESP32\\MenuManager.cpp"
...
char *MenuManager::getParentItemName(char *buf)
{
  *buf = 0;
  ...
  if (msi != 0)
  {
    buf = (char *)pgm_read_word_near(&(msi->menu[msi->itemIndexPos].name));
    // strcpy_P(buf, (char *)pgm_read_word_near(&(msi->menu[msi->itemIndexPos].name)));
  }
  return buf;
}
...
{
  return strcpy_P(buf, (char *)pgm_read_word_near(&(currentMenu[idx].name)));
}
...
{
  return strcpy_P(buf, (char *)pgm_read_word_near(&(currentMenu[currentMenuItemIndexPos].name)));
}
